...
```sql
SELECT
    skills.skill_id as skillid,
    skills.skills as skillname,
    ROUND(AVG(jobs.salary_year_avg), 2) as avg_salary,
    COUNT(jobs.job_id) as no_of_openings
FROM
    job_postings_fact jobs
INNER JOIN 
    skills_job_dim skillsjobs ON jobs.job_id = skillsjobs.job_id
INNER JOIN 
    skills_dim skills ON skillsjobs.skill_id = skills.skill_id
WHERE
    job_title_short = 'Data Analyst' AND
    job_work_from_home = 'TRUE' AND
    salary_year_avg IS NOT NULL
